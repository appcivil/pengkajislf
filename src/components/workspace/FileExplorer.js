      ...
      ${files.map(f => {
        ...
        return `
          <div class="folder-card ${isSelected ? 'active shadow-lg' : ''}" 
               ...
               onclick="window._handleWorkspaceSelect('${escapeHtml(f.id)}')"
               role="button"
               aria-label="File: ${escHtml(f.subcategory || f.name)}"
               tabindex="0"
               ondblclick="window.open('${escapeHtml(f.file_url)}', '_blank')">
            ...
            <div class="flex-center gap-1 mt-1">
               <div class="status-dot ${f.status === 'Final' ? 'green' : f.status === 'Needs Revision' ? 'red' : 'yellow'}" title="Status: ${escapeHtml(f.status)}"></div>
               <span style="font-size:0.6rem; font-weight:800; color:var(--tertiary)">${escapeHtml(f.completeness || 0)}%</span>
            </div>
          </div>
        `;
      }).join('')}
             ...
             ${files.map(f => {
                ...
                return `
                  <tr class="${isSelected ? 'bg-accent-bg' : ''} hover:bg-gray-50 cursor-pointer" 
                     ...
                     ` : ''}
                     <td style="padding:8px 12px">
                        <span class="badge ${f.status === 'Final' ? 'badge-success' : 'badge-warning'}" style="font-size:0.6rem">${escapeHtml(f.status || 'Draft')}</span>
                     </td>
                     <td style="padding:8px 12px">
                        <div class="flex items-center gap-2">
                           <div class="progress-wrap" style="width:60px; height:6px"><div class="progress-fill ${f.completeness > 80 ? 'green' : 'yellow'}" style="width:${escapeHtml(f.completeness)}%"></div></div>
                           <span class="font-bold" style="font-size:0.65rem">${escapeHtml(f.completeness)}%</span>
                        </div>
                     </td>
                     <td style="padding:8px 12px; opacity:0.6">${escapeHtml(f.category)}</td>
    ...
    <div class="column-view animate-fade-in" style="display:flex; height:100%; overflow-x:auto; background:white">
       ...
       <div style="width:200px; border-right:1px solid var(--border); overflow-y:auto">
          ...
          ${['umum','tanah','arsitektur','struktur','mep','lapangan'].map(c => `
            <div class="p-2 px-3 hover:bg-brand-50 cursor-pointer flex-between text-sm" onclick="window._handleWorkspaceNav('smart', '${escapeHtml(c)}')">
       ...
       <div style="flex:1; background:#f9fafb; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:40px">
          ${selectedId ? `
             <div class="animate-slide-in text-center">
                ...
                <button class="btn btn-primary btn-sm" onclick="window._openQuickLook('${escapeHtml(selectedId)}')">Quick Look (Space)</button>
             </div>
          ` : `
